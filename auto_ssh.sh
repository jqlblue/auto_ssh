...
while getopts ":hc:i:l:u:p:P:a" opt; do
    case $opt in
        c )
            __category=$OPTARG;;
        i )
...
d_set_profile() {
    local category=$1
    local profile="$($_current_dir/utility/profile.py -c $category)"
    _user="$(echo $profile | awk '{print $1}')"
    _pass="$(echo $profile | awk '{print $2}')"
    _port="$(echo $profile | awk '{print $3}')"
}
